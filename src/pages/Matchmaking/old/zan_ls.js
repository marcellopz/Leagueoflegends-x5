...
let blue_rank = 0, red_rank = 0;
for (let i = 0; i < 5; i++) {
	const p0 = solution[i * 2];
	const p1 = solution[i * 2 + 1];
	console.log(`${roles[i]}: ${p0.name} ${p0.ranks[i]} vs ${p1.name} ${p1.ranks[i]}`);

	blue_rank += p0.ranks[i];
	red_rank += p1.ranks[i];
}

console.log(`Blue team: ${blue_rank}`);
console.log(` Red team: ${red_rank}`);
console.log(`    Match: ${blue_rank + red_rank}`);
